...
char* searchForOperation(int arrayToSearch, int arrayIndex, const char* opcode) {
    ...
    if (arrayToSearch == 34) {
        ...
        printf("%s\t%c%s\t%s\n", outputLabel, plusExtension, opcodeName, operandValue);
        outputLabel[0] = '\0';
...
if(nixbpe[0][nIndex]==1 && nixbpe[1][nIndex]==1 && nixbpe[2][nIndex]==0 && nixbpe[3][nIndex]==0 && nixbpe[4][nIndex]==0 && nixbpe[5][nIndex]==0){//simple addressing type
    getDisp(0, recordLine, dispIndex, dispIndex+12); // you'll have constValue
...
if(nixbpe[0][nIndex]==1 && nixbpe[1][nIndex]==1 && nixbpe[2][nIndex]==0 && nixbpe[3][nIndex]==0 && nixbpe[4][nIndex]==0 && nixbpe[5][nIndex]==1){
    printf("+op m\n");// m will be addr format 4 instruction
    plusExtension = '+';
...
// 110010
if(nixbpe[0][nIndex]==1 && nixbpe[1][nIndex]==1 && nixbpe[2][nIndex]==0 && nixbpe[3][nIndex]==0 && nixbpe[4][nIndex]==1 && nixbpe[5][nIndex]==0){ // (pc)+disp
    getDisp(0, recordLine, dispIndex, dispIndex+12); // you'll have constValue
...
if(nixbpe[0][nIndex]==1 && nixbpe[1][nIndex]==1 && nixbpe[2][nIndex]==1 && nixbpe[3][nIndex]==1 && nixbpe[4][nIndex]==0 && nixbpe[5][nIndex]==0){
    getDisp(0, recordLine, dispIndex, dispIndex+12);
       sprintf(operandValue,"%d",displacement);
       if(displacement == programCounter){
           strncpy(outputLabel, symbols[symsUsedCounter], 10);
           symsUsedCounter++;
       }
...
if(nixbpe[0][nIndex]==0 && nixbpe[1][nIndex]==1 && nixbpe[2][nIndex]==0 && nixbpe[3][nIndex]==0 && nixbpe[4][nIndex]==0 && nixbpe[5][nIndex]==1){
    getDisp(0, recordLine, dispIndex+4, dispIndex+20); // you'll have constValue
    operandValue[0] = '#';
    plusExtension = '+';
    sprintf(operandValue+1,"%d",displacement);
    if(displacement == programCounter){
