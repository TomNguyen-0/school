...
David Arce: cssc0858@edoras.sdsu.edu (820099092)
Kellen Gillooley: cssc1024@edoras.sdsu.edu (818216237)
Logan Lasiter: cssc1034@edoras.sdsu.edu (820065721)
Tom Nguyen: cssc0883@edoras.sdsu.edu (820468110)
...
void parseHeader(char* headerRecord) {
    ...
    lengthOfProgram[7] = '\0';
    printAddr = strtol (startingAddress,&PAptr,16);
    fprintf(outfile,"%s\tSTART\t%x\n", nameOfProgram, printAddr);
        ...
        if(strcmp(opcodeName, "LDB") == 0)
         if(operandValue[0]=='#'){
            ...
            while(operandValue[k]!='\0'){
               fprintf(outfile,"%c",operandValue[k++]);
            }
           fprintf(outfile,"\n");
         }
        else
         fprintf(outfile,"\tBASE\t%s\n",operandValue);
...
char *getFilename(char *oldName) {
	size_t length = strlen(oldName);
	char *filename = malloc(length);
	memcpy(filename, oldName,length-3);
	filename[length-3] ='s';
	filename[length-2] = 'i';
	filename[length-1] = 'c';
	return filename;
